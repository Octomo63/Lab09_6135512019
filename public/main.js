    ...
    try {
        ...
        db.ref('contacts').on('child_added',snapshot => {
            if(snapshot.exists()) {
                ...
                let contactRecord = contactRecordFormSnapshot(snapshot);
        ...
        console.error(e);
        document.getElementById('status').innerHTML = 'Error loading the Firebase SDK, check the consloe.';
    }
});

function contactRecordFormSnapshot(snapshot) {
